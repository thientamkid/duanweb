...
<?php
	...
	$b_tv="select count(*) from game $chuoi_or";
	$b_tv_1=mysql_query($b_tv);
	$b_tv_2=mysql_fetch_array($b_tv_1);
	$st=ceil($b_tv_2[0]/$so_gioi_han);
	$chuoi="select * from game $chuoi_or order by id desc limit $vtbd,$so_gioi_han";
	//echo $chuoi."<hr>";
	$tv_1=mysql_query($chuoi);
?>
<center>
<form action="">
	<input type="hidden" name="thamso" value="quan_ly_game">
...
<table width="968px" style="text-align:left;margin-top:6px" id="er">
	...
	<?php
		while($tv_2=mysql_fetch_array($tv_1))
		{
			$link_hinh="../hinhanh_flash/game/$tv_2[hinh_anh]";
			$noi_dung_ngan=cat_chuoi_789(thay_the_fckeditor($tv_2['noi_dung']),0,150);
			$link_sua="?thamso=sua_game&id=$tv_2[id]&tu_khoa=$_GET[tu_khoa]&trang=$_GET[trang]";
			$link_xoa="?thamso=xoa_game&id=$tv_2[id]";
			?>
				<tr>
					<td align="center">
					...
					<td><span style="color:#0b55c4;font-size:14px"><?php echo $tv_2['loai'];?></span></td>
					<td><span style="color:#0b55c4;font-size:14px"><?php echo $tv_2['rong'];?></span></td>
					<td><span style="color:#0b55c4;font-size:14px"><?php echo $tv_2['cao'];?></span></td>
