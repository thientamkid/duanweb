...
<?php
	...
	{
		$tv="select * from menu where vitri_menu='ngang' and thuoc_menu='$id' order by id";
		$tv_1=mysql_query($tv);
	...
	$chuoi_union="";
	$tv="select * from menu where vitri_menu='ngang' and thuoc_menu='' order by id";
	$tv_1=mysql_query($tv);
	while($tv_2=mysql_fetch_array($tv_1))
	{
		$chuoi_union=$chuoi_union."( select * from menu where id='$tv_2[id]' ) union ";
	...
	/*tinh so trang*/
	$a_tv="select count(*) from menu where vitri_menu='ngang'";
	$a_tv_1=mysql_query($a_tv);
	$a_tv_2=mysql_fetch_row($a_tv_1);
	$st=ceil($a_tv_2[0]/$sd);
?>
<center>
	<table width="968px" style="margin-top:6px;text-align:left" id="er">
		<tr>
			<td width="768px">Tên menu</td>
		...
		<?php
			while($tv_2=mysql_fetch_array($tv_1))
			{
				...
				?>
					<tr>
						<td width="768px"><a href="<?php echo $link_sua; ?>" class="a_vvv"><?php echo $chuoi_kt;echo $tv_2['ten']; ?></a></td>
